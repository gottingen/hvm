...
class TestPytorchFloat16(unittest.TestCase):
    ...
    def test_float16(self):
        ...
        x = hvm.array.from_numpy(np.random.rand(3, 4).astype("float16"))
        h = hvm.array.from_numpy(np.random.rand(3, 4).astype("float16"))
        ...
        module_jit = hvm.trace(process, x, h)
        ...
        module_jit = hvm.load(save_path, device=-1)
        data = module_jit.run({'x': x, 'h': h})
        self.assertTrue(isinstance(data, tuple))
        self.assertTrue(isinstance(data[0], hvm.array.NDArray))
        self.assertSequenceEqual(data[0].shape(), (3, 4))
