...
# make torch op
my_cell = MyCell()
global_torch_op = hvm.script(my_cell)


class MyGenerator:

    def __init__(self, torch_op: Any, h: hvm.NDArray):
        self.torch_op: Any = torch_op
        self.h: hvm.NDArray = h
...
def test_control_flow():
    ...
    my_gen_op = hvm.script(MyGenerator)(torch_op=global_torch_op, h=h)
    ...
    jit_mod = hvm.load(save_path, device='cpu')
    data = jit_mod.run({'x': x})
    print(data)
