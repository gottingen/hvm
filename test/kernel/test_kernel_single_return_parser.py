...
class TestSingleReturnParser(unittest.TestCase):
    ...
    def test_multiple_bin_op_with_scalar_and_const(self):
        ...
        hvm_b = hvm.array.from_numpy(b)
        # hvm_c = hvm.array.from_numpy(c)
        # todo hvm.array.from_numpy does not support numpy constant
        rt = f(hvm_a, hvm_b, 3.0)  # not support np.float(3)
        np.testing.assert_equal(rt, foo(a, b, c))
