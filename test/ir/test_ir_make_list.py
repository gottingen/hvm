...
class TestIRMakeList(unittest.TestCase):
    ...
    def test_make_return_list(self):
        def make_return_list():
            ...
            def return_list():
                ...
                return result
            """
            alloca_result = hvm.ir.AllocaVarStmt("result", hvm.ir.ListType())
            ...
            ib.emit_expr(
                _ir.op.object_append(
                    ...
                    hvm.ir.const(
                        125,
                        "int64")))
            ib.emit(hvm.ir.ReturnStmt(alloca_result.var))
            return hvm.ir.Function([], [], ib.get(), hvm.ir.ListType())

        c_func = self.compile_func(make_return_list)
        result = c_func()
        # print(self.current_ir)
        # print(self.current_source)
        self.assertEqual(result, hvm.List([b"hello", 125]))
    ...
    def test_make_nested_list(self):
        def make_nested_list():
            ...
            """
            alloca_result = hvm.ir.AllocaVarStmt("result", hvm.ir.ListType())
            alloca_item0 = hvm.ir.AllocaVarStmt("item0", hvm.ir.ListType())
            alloca_item1 = hvm.ir.AllocaVarStmt("item1", hvm.ir.ListType())
        ...
        # print(self.current_ir)
        # print(self.current_source)
        self.assertEqual(result, hvm.List([hvm.List([b"hello", 125]), hvm.List([b"hi", 32])]))
