...
import hvm
from hvm.contrib.cc import create_shared
import os
import ctypes
import unittest
...
class _Lazy:
    ...
    def __init__(self, fu: Callable) -> None:
        self._fu: hvm.NativeObject = hvm.make_native_object("Future", fu)
...
class TestFutureInterface(unittest.TestCase):
    ...
    def test_py_future_eval(self):
        new_get_one = hvm.script(get_one)
        NewAddFive = hvm.script(AddFive)
        Lazy = hvm.script(_Lazy)
    ...
    def test_c_extension(self):
        ...
        if os.path.exists(output):
            os.remove(output)
        create_shared(output, ['my_future.cc'], ['-std=c++17', '-shared'] + hvm.get_cflags())
        ctypes.CDLL(output)

        get_ft = hvm.get_global_func("test.get_future")
        ft = get_ft()
