...
class WorkFlow:
    def __init__(self, init_value=10, script=True):
        ...
        if script:
            self.foo = hvm.script(Counter)(init_value)
        else:
...
class TestTracingClassMethod(unittest.TestCase):
    def test_tracing_class_method(self):
        ...
        workflow_hvm = WorkFlow(init_value=init_value, script=True)
        workflow_process_mod = hvm.trace(workflow_hvm.process, 1)
        ...
        for _ in range(test_steps):
            ...
            hvm_result = workflow_process_mod(a=random_int)

            self.assertEqual(py_result, hvm_result)
