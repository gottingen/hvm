...
class TestContainerTrie(unittest.TestCase):
    ...
    def test_runtime_build_trie(self):
        trie1 = hvm.Trie()
        trie2 = hvm.Trie(self.index_data)
    ...
    def test_constructor(self):
        @hvm.script
        def test_trie_constructor() -> hvm.Trie:
            trie1 = hvm.Trie()
            return trie1

        @hvm.script
        def test_trie_constructor_with_args() -> hvm.Trie:
            index_data = hvm.Dict()
            index_data["hello"] = 1
            index_data["hello world"] = 2
            trie2 = hvm.Trie(index_data)
            return trie2

        result = test_trie_constructor()
        self.assertTrue(isinstance(result, hvm.Trie))
    ...
    def test_update(self):
        ...
        self.assertTrue(isinstance(result, hvm.Trie))
        self.assertEqual((5, 1), result.prefix_search("hello"))
        result = test_trie_generic_update(hvm.Trie())
        self.assertTrue(isinstance(result, hvm.Trie))
        self.assertEqual((5, 1), result.prefix_search("hello"))
    ...
    def test_prefix_search(self):
        ...
        mblen, idx = test_trie_generic_prefix_search(hvm.Trie())
        self.assertEqual((5, 1), (mblen, idx))

    def test_prefix_search_all(self):
        def test_trie_prefix_search_all(trie: hvm.Trie, arg: Any) -> List:
            return trie.prefix_search_all(arg)

        def test_trie_generic_prefix_search_all(trie: Any, arg: Any) -> List:
            return trie.prefix_search_all(arg)

        op = hvm.script(test_trie_prefix_search_all)
        generic_op = hvm.script(test_trie_generic_prefix_search_all)
    ...
    def test_save_load(self):
        ...
        def test_trie_specific_save(trie: hvm.Trie, file_path: str) -> int:
            return trie.save(file_path)

        def test_trie_generic_save(trie: Any, file_path: str) -> int:
            return trie.save(file_path)

        def test_trie_specific_load(trie: hvm.Trie, file_path: str) -> int:
            return trie.load(file_path)

        def test_trie_generic_load(trie: Any, file_path: str) -> int:
            return trie.load(file_path)

        specific_save_op = hvm.script(test_trie_specific_save)
        generic_save_op = hvm.script(test_trie_generic_save)
        specific_load_op = hvm.script(test_trie_specific_load)
        generic_load_op = hvm.script(test_trie_generic_load)
        ...
        load_trie = hvm.Trie()
        specific_load_trie = hvm.Trie()
        generic_load_trie = hvm.Trie()
