...
class TestErrorReport(unittest.TestCase):
    def batch_assertRaisesRegex(self, expected_exception, expected_regex, funcs, *args, **kwargs):
        for func in funcs:
            self.assertRaisesRegex(expected_exception,
                                   expected_regex,
                                   lambda: hvm.script(func), *args, **kwargs)
    ...
    def test_assign_to_elem(self):
        ...
        with self.assertRaises(Exception):
            hvm.script(func2)
        self.assertEqual(hvm.script(func3)()[0], 1)
    ...
    def test_control_flow(self):
        ...
        with self.assertRaises(Exception):
            hvm.script(func_forget_return)
        with self.assertRaises(Exception):
            hvm.script(func_forget_return2)
        self.assertEqual(hvm.script(func_has_return)(), 2)
    ...
    def test_for(self):
        ...
        with self.assertRaises(Exception):
            hvm.script(zip_two_any)

        with self.assertRaises(Exception):
            hvm.script(zip_three_any)

        with self.assertRaises(Exception):
            hvm.script(enumerate_any)
    ...
    def test_subscript(self):
        ...
        with self.assertRaises(Exception):
            hvm.script(func_subscript_none)
        ...
        with self.assertRaises(Exception):
            hvm.script(func_subscript_float)
        ...
        with self.assertRaises(Exception):
            hvm.script(func_subscript_bool)
    ...
    def test_attribute(self):
        ...
        with self.assertRaises(Exception):
            hvm.script(func_attribute_none)
        ...
        with self.assertRaises(Exception):
            hvm.script(func_attribute_float)
        ...
        with self.assertRaises(Exception):
            hvm.script(func_attribute_bool)
        ...
        with self.assertRaises(Exception):
            hvm.script(func_attribute_tuple)
        ...
        with self.assertRaises(Exception):
            hvm.script(func_attribute_list)
        ...
        with self.assertRaises(Exception):
            hvm.script(func_attribute_dict)
    ...
    def test_autofor_error(self):
        ...
        outer, inner = hvm.script(autofor_list_append_0)(a)
        self.assertEqual(outer, [1, 2, 3, 0])
        self.assertEqual(inner, [4, 5, 0, 0, 0])

        with self.assertRaises(Exception):
            hvm.script(autofor_list_append_1)(a)

        with self.assertRaises(Exception):
            hvm.script(autofor_list_append_2)(a)

        with self.assertRaises(Exception):
            hvm.script(autofor_list_extend)(a)

        with self.assertRaises(Exception):
            hvm.script(autofor_list_remove)(a)

        with self.assertRaises(Exception):
            hvm.script(autofor_list_pop)(a)

        with self.assertRaises(Exception):
            hvm.script(autofor_list_popindex)(a)

        with self.assertRaises(Exception):
            hvm.script(autofor_dict_pop)(b)

        with self.assertRaises(Exception):
            hvm.script(autofor_set_add)(c)

        with self.assertRaises(Exception):
            hvm.script(autofor_set_update)(c)

        with self.assertRaises(Exception):
            hvm.script(autofor_set_discard)(c)

        with self.assertRaises(Exception):
            hvm.script(autofor_set_pop)(c)

        with self.assertRaises(Exception):
            hvm.script(autofor_set_remove)(c)
