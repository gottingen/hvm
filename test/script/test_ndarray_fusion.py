...
class TestNDArrayFusion(unittest.TestCase):
    ...
    def test_fusion_getitem(self):
        ...
        def fusion_getitem(init_values: List) -> int:
            s = 0
            a = hvm.NDArray(init_values, [2, 5], "int32")
            for i in range(2):
                for j in range(5):
                    s += a[i][j]
            return s
        ...
        exp_ret = sum(init)
        tx_ret = hvm.script(fusion_getitem)(init)
        self.assertEqual(tx_ret, exp_ret)

    def test_fusion_setitem(self):

        def fusion_setitem() -> hvm.NDArray:
            a = hvm.NDArray([], [2, 5], "int32")
            for i in range(2):
                for j in range(5):
                    a[i][j] = i + j
            return a

        tx_ret = hvm.script(fusion_setitem)().asnumpy()
        for i in range(2):
            for j in range(5):
