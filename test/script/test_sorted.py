...
from hvm.runtime import _ffi_api
from hvm.runtime.object_generic import to_runtime_object
...
class TestHvmListSorted(unittest.TestCase):
    ...
    def test_python_ftlist_sorted_reverse(self):
        @hvm.script
        def sorted_list(it: hvm.FTList, key: Any = None, reverse: bool = False) -> List[float]:
            return sorted(it, key, reverse)

        original: List[float] = [1.0, 3.0, 2.2, 5, 4.3, 2.15]
        expected: List[float] = sorted(original)
        original_copy: hvm.runtime.List = make_ft_list(original)
