...
class MyEchoOp1:
    __slots__: Tuple[hvm.NativeObject] = ['c_data']

    def __init__(self) -> None:
        self.c_data = hvm.make_native_object("MySimpleNativeDataExample")
...
class MyEchoOp2:
    ...
    def __init__(self, file_path: str) -> None:
        self.c_data = hvm.make_native_object("MyNativeDataExample", file_path)
...
class TestNativeUserData(unittest.TestCase):
    ...
    def test_runtime_native_data(self):
        nd = hvm.make_native_object("MySimpleNativeDataExample")
        print(nd.cls_name)
        print(nd.ud_ref)
    ...
    def test_runtime_native_data_resource(self):
        nd = hvm.make_native_object("MyNativeDataExample", self.test_file)
        print(nd.cls_name)
        print(nd.ud_ref)
        print(hasattr(nd, "get_content"))
        content = nd.get_content()
    ...
    def test_script_native_data(self):
        ...
        def trace_pipeline():
            op1 = hvm.script(MyEchoOp1)()
            op2 = hvm.script(MyEchoOp2)(self.test_file)
            res1 = op1()
            res2 = op2()
            return res1, res2
        ...
        jit_module = hvm.pipeline.Trace(trace_pipeline)
        res1, res2 = jit_module.run(feed_dict={})
        self.assertEqual((res1, res2), (ans1, ans2))

    def test_runtime_native_function(self):
        def native_function_call(a: str, b: int) -> str:
            r = hvm.make_native_function("MyNativeFunctionExample")(a.encode(), b)
            return r.decode()

        native_function_call_op = hvm.script(native_function_call)
    ...
    def test_native_module_function(self):
        def native_module_func(x: str) -> str:
            return hvm.native.MyNativeFunctionExample(x.encode(), 0).decode()

        def generic_native_module_func(x: str) -> str:
            return hvm.call_native_function(b'MyNativeFunctionExample', x.encode(), 0).decode()

        op = hvm.script(native_module_func)
        r = native_module_func("a")
        op_r = op("a")
        self.assertEqual("hello: a, 0", r)
        self.assertEqual("hello: a, 0", op_r)

        generic_op = hvm.script(generic_native_module_func)
        op_r = generic_op("a")
        self.assertEqual("hello: a, 0", r)
        self.assertEqual("hello: a, 0", op_r)
    ...
    def test_native_module_object(self):
        def native_module_object() -> str:
            obj = hvm.native.MySimpleNativeDataExample()
            return obj.get_content().decode()

        op = hvm.script(native_module_object)
