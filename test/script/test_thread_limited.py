...
class ParallelOp83:
    __slots__: Tuple[hvm.NativeObject] = ['thread_pool']

    def __init__(self, pool_size: int, use_lockfree_pool: bool) -> None:
        self.thread_pool = hvm.make_native_object(
...
class TestThreadPoolExecutorLimited(unittest.TestCase):
    ...
    def test_parallel_for(self):
        def pipeline1(inputs):
            op_1 = hvm.script(worker_func)

            pool_size = 3
            use_lockfree_pool = True
            parallel_op = hvm.script(ParallelOp83)(pool_size, use_lockfree_pool)

            outputs = parallel_op(op_1, inputs)
            return outputs

        def pipeline2(inputs):
            op_2 = hvm.script(WorkerOp)()

            pool_size = 3
            use_lockfree_pool = True
            parallel_op = hvm.script(ParallelOp93)(pool_size, use_lockfree_pool)

            outputs = parallel_op(op_2, inputs)
            return outputs

        self.assertEqual(pipeline1([1.1, 2.1, 3.1] * 2), hvm.List([2.2, 4.2, 6.2] * 2))
        self.assertEqual(pipeline2([1.1, 2.1, 3.1] * 2), hvm.List([2.2, 4.2, 6.2] * 2))
