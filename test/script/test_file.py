...
class TestFile(unittest.TestCase):
    ...
    def test_readline(self):
        ...
        scripted_output = hvm.script(readlines_string)(file_1)
        builtin_output = readlines_string(file_1)
        self.assertEqual(builtin_output, scripted_output)

        scripted_output = hvm.script(readlines_string)(file_2)
        builtin_output = readlines_string(file_2)
        self.assertEqual(builtin_output, scripted_output)

        scripted_output = hvm.script(readlines_bytes)(file_1)
        builtin_output = readlines_bytes(file_1)
        self.assertEqual(builtin_output, scripted_output)

        scripted_output = hvm.script(readlines_bytes)(file_2)
    ...
    def test_readlines(self):
        ...
        self.assertEqual(hvm.script(readlines_string)(path), readlines_string(path))
        self.assertEqual(hvm.script(readlines_bytes)(path), readlines_bytes(path))

    def test_multi_open(self):
        def share_open(path: str) -> hvm.List:
            ret = hvm.List()
        ...
        scripted_output = hvm.script(share_open)(path)
        builtin_output = share_open(path)
        self.assertEqual(builtin_output, scripted_output)

    def test_annotation(self):
        # TODO: Ann Iterable ?
        def readlines(f: hvm.File) -> List:
            ret = []
            for line in f:
                ret.append(line)
            return ret
    ...
    def test_file_read(self):
        ...
        tx_ret = hvm.script(file_read)(path)
