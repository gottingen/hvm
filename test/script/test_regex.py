...
class TestRegex(unittest.TestCase):
    ...
    def test_regex_compile(self):
        ...
        def run_regex() -> Tuple[List, Tuple[List, Dict], Tuple[List, Dict], str]:
            regex = hvm.Regex("name", False, False)
        ...
        assert len(token) == 2
        assert match_0 == hvm.Tuple(hvm.List(['name']), hvm.Dict())
        assert match_6 == hvm.Tuple(hvm.List(), hvm.Dict())
        ...
        assert matched_result == hvm.Tuple(hvm.List(
            ['Cats are smarter than dogs', 'Cats', 'smarter']),
            hvm.Dict({'first': 'Cats', 'second': 'smarter'}))

    def test_regex_annotate(self):
        @hvm.script
        def make_regex() -> hvm.Regex:
            regex = hvm.Regex("name", False, False)
            return regex

        regex = make_regex()
        assert regex.match("mynameisHe") == hvm.Tuple(hvm.List(['name']), hvm.Dict())

    def test_regex_for_archer(self):
        @hvm.script
        def make_whitespace_regex() -> hvm.Regex:
            regex = hvm.Regex(r'[ \t\n\r\f\p{Zs}]', ignore_case=False, dotall=False)
            return regex

        @hvm.script
        def make_control_regex() -> hvm.Regex:
            regex = hvm.Regex(r'[\u0000\ufffd\p{Cc}\p{Cf}\p{Mn}]', ignore_case=False, dotall=False)
            return regex
    ...
    def test_regex_type_convert(self):
        ...
        def clean_string(ss: str) -> str:
            ...
            for tup in replace_pair:
                replaces.append((hvm.Regex(tup[0], False, False), tup[1]))
            ss1 = ss
            for tup in replaces:
    ...
    def test_special_replace(self):
        ...
        tx_ret = hvm.script(replace_with_empty)(text)
        ...
        tx_ret = hvm.script(replace_with_0)(text)
        self.assertEqual(py_ret, tx_ret)
