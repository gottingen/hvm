...
class TestContainerStr(unittest.TestCase):
    ...
    def test_list_slice_isdigit(self):
        @hvm.script
        def list_slice_isdigit(s: hvm.List) -> bool:
            return s[0].isdigit()

        self.assertTrue(list_slice_isdigit(hvm.List(["123"])))
    ...
    def test_split(self):
        @hvm.script
        def str_split(s: str, sep: Any, maxsplit: int) -> hvm.List:
        ...
        @hvm.script
        def str_split_generic(slist: hvm.List, sep: str, maxsplit: int) -> hvm.List:
            ret = hvm.List()
            for s in slist:
        ...
        self.assertEqual(
            ...
            hvm.List('1ab2ab3ab4'.split('ab', 1))
        )
        self.assertEqual(
        ...
        with self.assertRaises(Exception):
            hvm.script(invalid_str_ft_split)
    ...
    def test_join(self):
        ...
        @hvm.script
        def str_join_list(s: str, __list: hvm.List) -> str:
        ...
        @hvm.script
        def object_join_list(s: Any, __list: hvm.List) -> str:
            return s.join(__list)
    ...
    def test_startswith(self):
        ...
        with self.assertRaises(Exception):
            hvm.script(str_startswith_tup_except)('hhabc')
        ...
        with self.assertRaises(Exception):
            hvm.script(bytes_startswith_tup_except)(b'hhabc')
    ...
    def test_ord(self):
        ...
        bs = b'a'
        ret = ord(s)
        hvm_ret = hvm.script(str_ord)(s)
        bret = ord(bs)
        bhvm_ret = hvm.script(bytes_ord)(bs)
        self.assertEqual(ret, hvm_ret)
        self.assertEqual(bret, bhvm_ret)

    def test_chr(self):
        def str_chr(i: int) -> str:
            return chr(i)

        scripted_func = hvm.script(str_chr)
        for i in range(0x9FEF):
            self.assertEqual(scripted_func(i), str_chr(i))

    def test_format(self):
        def str_format() -> hvm.List:
            ret = hvm.List()
            s = 'abc'
            l = hvm.List(["aaa", b"bbb"])
            ret.append('{{}}123')
            ret.append('11{}22')
        ...
        self.assertEqual(str_format(), hvm.script(str_format)())
        self.assertRaises(Exception, hvm.script(str_illegal_format1))
        self.assertRaises(Exception, hvm.script(str_illegal_format2))
        self.assertRaises(Exception, hvm.script(str_illegal_format3))

        def generic_format(s: Any, param: Any) -> str:
            return s.format(param)

        self.assertEqual(generic_format('p: {}', hvm.Dict({1: 2})),
                         hvm.script(generic_format)('p: {}', hvm.Dict({1: 2})))
