...
class TestThreadPoolExecutor(unittest.TestCase):
    ...
    def test_parallel_for(self):
        def pipeline1(inputs):
            ...
            parallel_op = hvm.script(ParallelOp1)(pool_size, use_lockfree_pool)
        ...
        def pipeline2(inputs):
            ...
            pool_size = 3
            use_lockfree_pool = True
            parallel_op = hvm.script(ParallelOp2)(pool_size, use_lockfree_pool)

            outputs = parallel_op(op_2, inputs)
            return outputs

        self.assertEqual(pipeline1([1.1, 2.1, 3.1]), hvm.List([2.2, 4.2, 6.2]))
        self.assertEqual(pipeline2([1.1, 2.1, 3.1]), hvm.List([2.2, 4.2, 6.2]))
...
class TestThreadPoolWithException(unittest.TestCase):
    ...
    def test_parallelfor_exception(self):
        ...
        def run_function() -> None:
            thread_pool = hvm.make_native_object("ThreadPoolExecutor", 4, True)
