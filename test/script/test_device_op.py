...
cpu_device = hvm.Device("cpu")
gpu_device = hvm.Device("gpu:0")
default_device = hvm.Device("")


class PrefixDeviceOp:
    def __init__(self, device: Any = None) -> None:
        self.func: Any = hvm.make_native_object("MyDeviceOpExample", device())
...
class TestDeviceOpDict(unittest.TestCase):
    ...
    def test_script_use(self):
        creator = hvm.script(PrefixDeviceOp)
    ...
    def test_session_use(self):
        ...
        session = hvm.pipeline.Trace(process, b"haha")
        session.save("device_op_session")
        t_session = hvm.load("device_op_session", -1)
    ...
    def test_script_device(self):
        def _make_device() -> Any:
            d: hvm.Device = hvm.Device("cpu")
            c: hvm.pipeline.ops.DeviceOp = hvm.Device("cpu")
            return d, c

        hvm.script(_make_device)()
