...
class TestJson(unittest.TestCase):
    ...
    def test_consistency(self):
        ...
        d = {"\u6211": [2, 3], "\u4f60": {"x": 4}}
        s = json.dumps(d)
        self.assertEqual(json_loads_func(s), hvm.Dict(d))

        s = json_dumps_func(hvm.Dict(d))
        self.assertEqual(json.loads(s), d)

    def test_runtime_json(self):
        d = {"\u6211": [2, 3], "\u4f60": {"x": 4}}
        s = json.dumps(hvm.Dict(d))
        self.assertEqual(json.loads(s), d)

    def test_fused_json(self):
        d = {"\u6211": [2, 3], "\u4f60": {"x": 4}, "cc": (1, "s")}
        pure_d = {'d': d}
        fused_d = {'d': hvm.Dict(d)}
    ...
    def test_float_inf(self):
        ...
        d = hvm.serialize(data)
        print(d)
        d2 = hvm.deserialize(d)
        print(d2)
