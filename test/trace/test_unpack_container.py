...
class TestUnpackContainer(unittest.TestCase):
    ...
    def test_unpack_list(self):
        ...
        def tx_pipeline():
            a, b, c = hvm.script(return_3_results)()
            return a, b, c

        py_ret = py_pipeline()
        mod = hvm.trace(tx_pipeline, )
        tx_ret = mod.run({})
        self.assertEqual(py_ret, tx_ret)
        save_path = self.work_path + "/test_unpack_list"
        mod.save(save_path)

        mod = hvm.load(save_path, "cpu")
        tx_ret = mod.run({})
        self.assertEqual(py_ret, tx_ret)
    ...
    def test_unpack_dynamic(self):
        ...
        def my_loop_fn(n: int) -> Any:
            cons = hvm.script(make_list)(n)
            ret = []
            for x in cons:
                ret.append(x)
            return tuple(ret)

        jit_mod = hvm.trace(my_loop_fn, 3)
        ...
        def my_unpack_fn(n: int) -> Any:
            a, d, h = hvm.script(make_list)(n)
            return a, d, h

        jit_mod = hvm.trace(my_unpack_fn, 3)
