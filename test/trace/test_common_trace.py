...
class TestCommonTrace(unittest.TestCase):
    ...
    def test_session_attr(self):
        ...
        jit_module.set_sess_attr("allowed_batch_size", hvm.List([4, 8, 16]))
        ...
        new_jit_mod = hvm.pipeline.Load(save_path, -1)
    ...
    def test_trace_container_constructor(self):
        ...
        m1 = hvm.trace(process, 20, 10)
        self.assertEqual(m1.run({"a": 20, "b": 10}), 224)
        m1.save(save_path)
        m2 = hvm.load(save_path, "cpu")
    ...
    def test_trace_container_getitem_nested(self):
        @hvm.script
        def make_ndarray(dtype: str) -> hvm.NDArray:
            return hvm.NDArray([1, 2, 3, 4, 5, 6, 7, 8], [2, 2, 2], dtype)
    ...
    def test_make_session_by_one_function(self):
        ...
        sess = hvm.toolchain.make_session(add)
    ...
    def test_make_session_by_one_class(self):
        ...
        sess = hvm.toolchain.make_session(AddX)(7)
    ...
    def test_make_session_by_one_class_method(self):
        ...
        sess = hvm.toolchain.make_session(AddX, method='add')(7)
        assert sess.run({'x': 2}) == 9
        assert sess.input_names == ['x']
