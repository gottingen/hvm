...
class TestClassAsPipelineArgs(unittest.TestCase):
    ...
    def test_op_process_class(self):
        ...
        def trace_pipeline(ud1: MyInfo):
            op1 = hvm.script(MyOp1)()
            op2 = hvm.script(MyOp2)()
            res1 = op1(ud1)
            res2 = op2(ud1)
            return res1, res2
        ...
        compiled_ud = ud_creator(1, 1, 1)
        jit_module = hvm.pipeline.Trace(trace_pipeline, ud1=compiled_ud)
        compiled_ud = ud_creator(1, 1, 1)
        ...
        jit_module = hvm.pipeline.Load(self.module_path, -1)
        compiled_ud = ud_creator(1, 1, 1)
        res1, res2 = jit_module.run(feed_dict={"ud1": compiled_ud})
        print(res1, res2)
        self.assertEqual((res1, res2), (ans1, ans2))
    ...
    def test_op_process_generic_class(self):
        ...
        def trace_pipeline(ud1: MyInfo):
            op_sum = hvm.script(MyOpSum)()
            op1 = hvm.script(MyOp3)()
            op2 = hvm.script(MyOp3)()
            res1 = op1(ud1)
            res2 = op2(ud1)
            res = op_sum(res1, res2)
            return res
