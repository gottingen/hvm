...
def record_build_info():
    ...
    if CUDA_HOME is not None:
        ...
        build_info = "Build Hercules with CUDA {}.".format(nvcc_version.strip())
    else:
        build_info = "Build Hercules without CUDA."

    build_info_file = os.path.join(BASE_DIR, "hvm", "build_info")
    if not os.path.exists(build_info_file):
        with open("hvm/build_info", 'w') as fw:
            print(build_info)
            fw.write(build_info.strip())
    else:
        print("hvm/build_info already exists.")
