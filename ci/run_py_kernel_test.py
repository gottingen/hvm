...
###############################################################################
TEST_KERNEL_PATH = os.path.join(ROOT_PATH, 'test/kernel')
os.chdir(TEST_KERNEL_PATH)

test_files = []
for a, b, c in os.walk('./'):
    for cc in c:
        if cc.startswith('test_') and cc.endswith('.py') and "cuda" not in cc:
...
def worker(test_file):
    env = os.environ.copy()
    env['HVM_DSO_DIR'] = test_file + '_dso'
